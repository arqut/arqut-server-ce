...
func TestHandleServiceSync(t *testing.T) {
	...
	t.Run("update service owned by different edge", func(t *testing.T) {
		mockStorage.ExpectedCalls = nil
		mockConn.sentMessages = nil

		existingService := &models.EdgeService{
			ID:         "svc-1",
			EdgeID:     "different-edge", // Different edge owns this service
			Name:       "Old Name",
			TunnelPort: 8080,
			LocalHost:  "localhost",
			LocalPort:  3000,
		...
		// Verify error ack was sent due to ownership mismatch
		assert.Equal(t, 1, len(mockConn.sentMessages))
		ackData := mockConn.sentMessages[0].Data.(map[string]interface{})
		assert.Equal(t, "error", ackData["status"])
	})

	t.Run("delete service with storage error", func(t *testing.T) {
		mockStorage.ExpectedCalls = nil
		mockConn.sentMessages = nil
		...
		mockStorage.On("DeleteEdgeService", "svc-1").Return(errors.New("database error"))
	...
	t.Run("update with storage error", func(t *testing.T) {
		mockStorage.ExpectedCalls = nil
		mockConn.sentMessages = nil
		...
		mockStorage.On("GetEdgeService", "svc-1").Return(existingService, nil)
		mockStorage.On("UpdateEdgeService", mock.AnythingOfType("*models.EdgeService")).
...
func TestHandleServiceSyncBatch(t *testing.T) {
	...
	t.Run("batch sync with existing services updates them", func(t *testing.T) {
		mockStorage.ExpectedCalls = nil
		mockConn.sentMessages = nil

		existingService := &models.EdgeService{
			ID:         "svc-ex", // Max 8 chars
			EdgeID:     edgeID,
			Name:       "Old Name",
			TunnelPort: 8080,
			LocalHost:  "localhost",
		...
		// Existing service should be updated
		mockStorage.On("GetEdgeService", "svc-ex").Return(existingService, nil)
		mockStorage.On("UpdateEdgeService", mock.AnythingOfType("*models.EdgeService")).Return(nil)
	...
	t.Run("batch sync create fails after update fails", func(t *testing.T) {
		mockStorage.ExpectedCalls = nil
		mockConn.sentMessages = nil

		services := []interface{}{
			map[string]interface{}{
				"id":          "svc-f", // Max 8 chars
		...
		// Update fails (not found), then create also fails
		mockStorage.On("GetEdgeService", "svc-f").Return(nil, errors.New("not found"))
		mockStorage.On("CreateEdgeService", mock.AnythingOfType("*models.EdgeService")).
		...
		// Verify ack was sent (synced 0 services)
		assert.Equal(t, 1, len(mockConn.sentMessages))
		ackData := mockConn.sentMessages[0].Data.(map[string]interface{})
		assert.Equal(t, "success", ackData["status"])
		assert.Contains(t, ackData["message"], "Synced 0 services")
	})
}
